...
// GenericPodChainer is terway chain object
type GenericPodChainer struct{}

// ImplementsAdd returns true if method 'add' is available
func (f *GenericPodChainer) ImplementsAdd() bool {
	return true
}

// Add setups the ipvlan port's tc-bpf
func (f *GenericPodChainer) Add(ctx context.Context, pluginCtx chainingapi.PluginContext) (res *cniTypesVer.Result, err error) {
	...
	// set bpf
	mapFD, mapID, err = connector.SetupInterfaceInRemoteNsWithBPF(netNs, ifName, true, true)
	if err != nil {
	...
	ep := &models.EndpointChangeRequest{
		...
		Mac:            containerMac,
		// for generic pod mode, host mac is meaningless, set a empty mac
...
func (f *GenericPodChainer) ImplementsDelete() bool {
	return true
}

// Delete deletes cilium endpoint
func (f *GenericPodChainer) Delete(ctx context.Context, pluginCtx chainingapi.PluginContext) (err error) {
...
func init() {
	chainingapi.Register(name, &GenericPodChainer{})
}
